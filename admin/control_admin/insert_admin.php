...
if (isset($_POST['nombre'])) {
    ...
    if ($nombre && $apellidos && $direccion && $poblacion && $provincia && $codigo_postal && $telefono && ($email && !$resul_email) && $fecha_nac && $sexo && ($usuario && !$resul_user) && $pass) {
        ...
        $guardado = "<h3 class = 'registro_ok'>Se han registrado correctamente el nuevo usuario.</h3>";
        
        header("location: ../../views/admin_user.php?ok=$guardado");
    } else {
        if (empty($nombre)) {
            $errores[0] = true;
        } else {
        ...
        header('location: ../../views/crear_user_admin.php?nombre=' . $errores[0] . '&apellidos=' . $errores[1] . '&direccion=' . $errores[2] . '&poblacion=' . $errores[3] . '&provincia=' . $errores[4] . '&codigo_postal=' . $errores[5] . '&telefono=' . $errores[6] . '&email=' . $errores[7] . '&fecha_nac=' . $errores[8] . '&sexo=' . $errores[9] . '&usuario=' . $errores[10] . '&pass=' . $errores[11]);
    }
