...
if (isset($_SESSION['id'])){
    ...
    //Podría utilizar filtros para número y el email, pero he obtado mejor por expresiones       
    ...
    $errores = [false, false, false, false, false, false, false, false, false, false, false];    

    if ($nombre && $apellidos && $direccion && $poblacion && $provincia && $codigo_postal && $telefono && ($email && !$resul_email) && $fecha_nac && $sexo && ($passo_V || $passo_in=='')) {               
        ...
        header("location: ../../views/perfil_use.php?ok=$guardado&nombre=$nombre&apellidos=$apellidos&direccion=$direccion&poblacion=$poblacion&provincia=$provincia&cp=$codigo_postal&tel=$telefono&email=$email&fecha=$fecha_nac&sexo=$sexo&ok2=$guardado_pass");
    } else {
        if (empty($nombre)) {
            $errores[0] = true;
        } else {
        ...
        header('location: ../../views/perfil_use.php?nombre=' .$errores[0].'&apellidos=' . $errores[1] . '&direccion=' . $errores[2] . '&poblacion=' . $errores[3] . '&provincia=' . $errores[4] . '&cp=' . $errores[5] . '&tel=' . $errores[6] . '&email=' . $errores[7] . '&fecha=' . $errores[8] . '&sexo=' . $errores[9].'&ok='.$fallo);
