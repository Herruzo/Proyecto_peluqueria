...
<head>
    ...
    <title>Perfil usuario admin</title>
...
<body>
    ...
    if (!isset($_SESSION['usuario']) || $_SESSION['usuario'] != 'admin') {
        ...
        if (!isset($_GET['idU'])){
    //Obtenemos el id del usuario seleccionado, recibido desde admin_user por GET
            $id = filter_input(INPUT_GET, 'id', FILTER_SANITIZE_FULL_SPECIAL_CHARS);/* $_GET['id']; */
        }else{
            $id = $_SESSION['idU'];
        }
    $nombre = $_GET['nombre'];       
    ...
    $rol = filter_input(INPUT_GET, 'rol', FILTER_SANITIZE_FULL_SPECIAL_CHARS);
    //Recibimos el id de consulta_todos_perfiles.php y mandamos $idU al input hidden con id="idU.
    $idU = filter_input(INPUT_GET, 'idUse', FILTER_SANITIZE_FULL_SPECIAL_CHARS);
    $observaciones = filter_input(INPUT_GET, 'observaciones', FILTER_SANITIZE_FULL_SPECIAL_CHARS);

    if(!$nombre){//Si no existe nombre, le mandamos el id recibido del admin_user.php a consulta_todos_perfiles.php
        header("location: ../admin/control_admin/consulta_todos_perfiles.php?id=$id");
    } 
    $id_retorno = filter_input(INPUT_GET, 'id_modi', FILTER_SANITIZE_FULL_SPECIAL_CHARS);
    ...
    $text_er_7 = "Tienes que elegir una opción.";
    }
    ?>
    ...
    <div class="contenedor_main">
        <div class="contenedor_main">            
            <form action="../admin/control_admin/modificar_todos_perfiles.php" method="POST" id="perfil" name="perfil" class="perfil" >
                <fieldset id= "grupo_datos">
                    <legend id="legend_datos">
                        <h3>&nbsp;Estos son los datos de <?php echo $nombre?>&nbsp;, puedes hacer cambios</h3>
                    </legend>
                    <table>
                        <tr><td><input type="hidden" name="idU" id="idU" value="<?php echo ($idU == '')?$id_retorno:$idU;?>"></td></tr>
                        ...
                        <tr class="position_input tr_form">
                            <td class="td_form"><label for="rol">Rol: </label></td>
                            <td class="td_form">
                                <select name="rol" id="rol" class="inp_form">
                        ...
                        <tr class="position_input tr_form">
                            <td class="td_form"><label for="observacones">Observaciones: </label></td>
                            <td class="td_form">
                                <textarea class="inp_form" name="observaciones" id="observaciones" cols="22" rows="6" value="<?php echo ($observaciones == 1) ? '' : $observaciones  ?>"><?php echo ($observaciones == 1) ? '' : $observaciones  ?></textarea>
                            </td>
                        </tr>
                    </table>  
                </fieldset>
                    ...
                    <table>                        
                        <tr class="position_input tr_form">
                            ...
                            <td class="td_form"><input class="inp_form" type="password" name="pass" id="pass" >
                                ...
                                <span class="ver" ><img class="ico_ver" src="../assets/images/ojo.png" id="ver_pass" alt="ver password" title="Ver contraseña"></span> <br>
                                <small>Error mensaje</small>                                
                            </td>
                        </tr>
                        <tr class="position_input tr_form">
                ...
                <div class="botones">
                    ...
                    <a href="#"><button type="button" name="baja" class="boton_ir_registro loging" id="baja">Eliminar usuario</button></a>
                </div>
            </form>
        </div>
    </div>
