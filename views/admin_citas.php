...
<body>
        ...
        <nav id="nav_menu" class="nav_menu">
            <div id="nav_links_registro" class="nav_links_registro">
                <div class="nav-links comun">
                    ...
                    <a class="link-item" href="#" id="op1">Admin citas</a>
    ...
    <div class="contenedor_main">
            <div class="crear_user">
                <a href="./mostrar_usuarios.php"><button type="submit" name="admin_cita" class="solicitar_cita" id="boton_log">Crear una nueva cita</button></a>
            </div>
            <br>
            <span>
                <?php
            ...
            <div>
                <h2><center>Estas son las citas que tenemos</center></h2>
            </div>
            <br>
            <div class="mostrar_user" id="admin_citas">
            <table>
                    <tr>
                        <th>Usuario</th>
                        <th>Nombre</th>
                ...
                $consulta_limite= "SELECT * FROM user_data INNER JOIN user_login ON(user_login.id_user_FK = user_data.idUser) INNER JOIN citas ON (citas.id_user_FK = user_data.idUser) WHERE fecha_cita > CURDATE() OR fecha_cita = CURDATE() ORDER BY fecha_cita, hora_cita LIMIT $empezar,$tamagno_parinas";
                ...
                while($fila = $ver_user->fetch(PDO::FETCH_ASSOC)){
                    ...
                    echo '<tr class="borde">
                    <td class ="td_admin borde"><center>'.$fila['usuario'].'</center></td>
                    <td class ="td_admin borde"><center>'.$fila['nombre'].'</center></td>
                    <td class ="td_admin borde"><center>'.$fila['apellidos'].'</center></td>
                    <td class ="td_admin borde"><center>'.$fila['fecha_cita'].'</center></td>
                    <td class ="td_admin borde"><center>'.$fila['hora_cita'].'</center></td>
                    
                    <td><a href="../admin/control/control_modificar_cita.php?idU='.$fila['idUser'].'&fecha='.$fila['fecha_cita'].'&hora='.$fila['hora_cita'].'&servicio='.$fila['servicio'].'&iD='.$fila["idCita"].'"><img class="ico_editar" src="../assets/images/editar_3_c.png" title="Modificar" alt="editar"></a></td>

                    <td><a href="../admin/control/baja_cita.php?idU='.$fila['idUser'].'&iD='.$fila['idCita'].'"><img class="ico_borrar" src="../assets/images/borrar_1_c.png" title="Eliminar" alt="eliminar"></a></td>
                    </tr>';
